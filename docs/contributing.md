...
Report bugs at https://github.com/osl-incubator/xh.git/issues.
...
Xhell could always use more documentation, whether as part of the official Xhell
docs, in docstrings, or even on the web in blog posts, articles, and such.

### Submit Feedback

The best way to send feedback is to file an issue at
https://github.com/osl-incubator/xh.git/issues.
...
Ready to contribute? Here’s how to set up `xh` for local development.

1. Fork the `xh` repo on GitHub.
2. Clone your fork locally and change to the directory of your project:

```bash
$ git clone git@github.com:your_name_here/xh.git
$ cd xh/
```
...
```bash
$ git remote add upstream https://github.com/osl-incubator/xh.git
$ git fetch --all
```
...
- `xh` uses a set of `pre-commit` hooks to improve code quality. The hooks can
...
The tests can be executed using the `test` dependencies of `xh` in the following
way:
